...
import FindMyWay from "find_my_way";
import Context from "./context.js";
import handleRequest from "./handleRequest.js";
import { hookRunner, hookIterator, lifecycleHooks } from "./hooks.js";
...
import { parseHeadOnSendHandlers } from "./headRoute.js";
...
function buildRouting(options) {
  ...
  function route(options) {
    ...
    if (!opts.handler) {
      throw new Error(
        `Missing handler function for ${opts.method}:${opts.url} route.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Error Handler for ${opts.method}:${opts.url} route, if defined, must be a function`
      );
    }
...
export default { buildRouting, validateBodyLimitOption };
