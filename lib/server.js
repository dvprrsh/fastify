...
import { kState, kOptions } from "./symbols.js";
import {
...
function createServer(options, httpHandler) {
  ...
  if (options.serverFactory) {
    ...
    if (options.http2) {
      ...
      server = http.createServer(options.https, httpHandler);
  ...
  function listen() {
    const normalizeListenArgs = (args) => {
      ...
      if (typeof firstArg === "object" && firstArg !== null) {
        ...
        // Fixes http://github.com/fastify/fastify/issues/1022.
    ...
    const listenPromise = (listenOptions) => {
      ...
      return this.ready().then(() => {
        let errEventHandler;
        const errEvent = new Promise((_resolve, reject) => {
          errEventHandler = (err) => {
            this[kState].listening = false;
            reject(err);
          };
        ...
        const listen = new Promise((resolve, _reject) => {
