...
import { finished as eos } from "node/stream.ts";
import { STATUS_TEXT as statusText } from "http/mod.ts";
import flatstr from "flatstr";
import FJS from "fast_json_stringify";
import {
  kSchemaResponse,
  kFourOhFourContext,
...
import { hookRunner, hookIterator, onSendHookRunner } from "./hooks.js";

const internals = require("./handleRequest")[Symbol.for("internals")];
import { now as _now } from "./logger.js";
const now = _now;
    ...
    if (hasContentType === false) {
      ...
      if (contentType.indexOf("charset") === -1) {
        ...
        if (contentType.indexOf("/json") > -1) {
          ...
          this[kReplyHeaders]["content-type"] = CONTENT_TYPE.JSON.replace(
            "/json",
            customJsonType
          );
        }
...
Reply.prototype.serialize = function (payload) {
  if (this[kReplySerializer] !== null) {
    ...
    if (this.context && this.context[kReplySerializerDefault]) {
      return this.context[kReplySerializerDefault](
        payload,
        this.raw.statusCode
      );
    } else {
...
function preserializeHookEnd(err, _request, reply, payload) {
  if (err != null) {
  ...
  if (reply[kReplySerializer] !== null) {
    ...
    payload = reply.context[kReplySerializerDefault](
      payload,
...
function wrapOnSendEnd(err, _request, reply, payload) {
  if (err != null) {
    onErrorHook(reply, err);
  } else {
    onSendEnd(reply, payload);
...
function onErrorHook(reply, error, cb) {
  reply[kReplySent] = true;
  if (
    reply.context.onError !== null &&
    reply[kReplyErrorHandlerCalled] === true
  ) {
    reply[kReplyIsRunningOnErrorHook] = true;
    onSendHookRunner(reply.context.onError, reply.request, reply, error, () =>
  ...
  try {
    ...
    payload =
      serializerFn === false
        ...
        : serializerFn(
            Object.create(error, {
              error: { value: statusText[statusCode + ""] },
...
function onResponseCallback(err, _request, reply) {
...
const _buildReply = buildReply;
export { _buildReply as buildReply };
const _setupResponseListeners = setupResponseListeners;
export { _setupResponseListeners as setupResponseListeners };
